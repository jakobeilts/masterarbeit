...
# Verwende "rank_zephyr" (lokal, erfordert mind. 24 GB GPU) ODER "gpt" (nutzt GPT-Reranking via API)
compressor = RankLLMRerank(
    top_n=4,  # Wie viele Dokumente nach Reranking weitergegeben werden
    model="gpt",  # Alternativ: "rank_zephyr"
    gpt_model="gpt-4o-mini"  # Nur nötig, wenn model="gpt"
)

# Ursprünglicher Retriever bleibt FAISS
base_retriever = vector_store.as_retriever(search_type="similarity", search_kwargs={"k": 20})

# ContextualCompressionRetriever mit RankLLMRerank als Kompressor
