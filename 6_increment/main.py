...
# ============================================================================
# main.py – StudIT RAG‑Chat   (Parent‑DocumentRetriever integriert)
# ============================================================================
...
SYSTEM_PROMPT = (
    ...
    "• Falls du die Antwort nicht sicher weißt, schlage konkrete Anlaufstellen (Link oder E‑Mail) vor.\n"
...
# --- Parent‑DocStore ----------------------------------------------------------
DOCSTORE_PATH = pathlib.Path("parent_docstore.pkl")
if DOCSTORE_PATH.exists():
    docstore = pickle.loads(DOCSTORE_PATH.read_bytes())
else:
    st.warning("⚠️  parent_docstore.pkl nicht gefunden –  DocStore leer.")
    docstore = InMemoryStore()

# ----------------------------------------------------------------------------
# 3) Parent‑Retriever  (+ Rerank‑Kompression)
# ----------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------
# 4) LLM, Memory & ConversationalRetrievalChain
# ----------------------------------------------------------------------------
    ...
    if sources:
        with st.expander("🔗 Quellen anzeigen"):
            for i, doc in enumerate(sources, 1):
                label = doc.metadata.get("url", "(keine URL)")
